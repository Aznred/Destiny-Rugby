...
const rencontre = (horloge: number, domicile: number, exterieur: number, termine = false) => ({
  ...
  match: { id: 'match-1', instance: 1_700_000_000_000, horloge, minute: Math.floor(horloge), termine, score: { domicile, exterieur } },
});
...
reinitialisee.rencontres[0].match!.instance = 1_800_000_000_000;
...
const photographieAncienne = vue(13, 20, 7, 3);
assert.equal(fusionnerVueLigue(actuelle, photographieAncienne).rencontres[0].match?.horloge, 62.5, 'Une nouvelle version de ligue ne doit pas rajeunir le même match');
