...
// dure (voir CLAUDE.md). Un succès = 1 à 16 Ovas, un défi = 1 à 2.
//
// ⚠️ RÉÉTALONNÉ À LA DEMANDE DE L'UTILISATEUR : « si on fait une bonne carrière
// avec les achievements on fait facile 4 000-5 000 Ovas, réduis pour que ça
// soit plus autour des 500. » Toutes les valeurs de `SUCCES` ont été divisées
// par 2,5 (cagnotte totale 1 060 → 424) et celles des trophées par 2
// (`data/trophees.ts`, 445 → 234). La vraie fuite, elle, était ailleurs : les
// DÉFIS DE LA SEMAINE, qui payaient sans plafond — ils le sont maintenant
// (`PLAFOND_OVAS_DEFIS_PAR_SAISON`, store). Budget visé pour une belle carrière
...
export const SUCCES: Succes[] = [
  ...
  {
    id: 'doublette_saison', emoji: '💫', nom: 'Le doublé', ovas: 10, secret: true,
  ...
  {
    id: 'six_nations', emoji: '🎽', nom: 'Vainqueur du Tournoi', ovas: 8,
    desc: 'Remporter le Tournoi des 6 Nations avec ta sélection.',
    atteint: (c) => aGagne(c, 'sixNations'),
  },
  {
    id: 'coupe_du_monde', emoji: '🏆', nom: 'Champion du monde', ovas: 16, secret: true,
    desc: 'Soulever la Coupe du monde.',
    atteint: (c) => aGagne(c, 'monde'),
  },
  {
    id: 'meilleur_joueur', emoji: '🥇', nom: 'Meilleur joueur du monde', ovas: 14, secret: true,
    desc: 'Être élu meilleur joueur du monde.',
    atteint: (c) => aGagne(c, 'meilleurJoueur'),
  },
  {
    id: 'palmares_deux_clubs', emoji: '🧳', nom: 'Champion partout', ovas: 7,
    desc: 'Remporter un titre avec deux clubs différents.',
    atteint: (c) => clubsTitres(c).size >= 2,
  },
  {
    id: 'palmares_trois_clubs', emoji: '🗺️', nom: 'Mercenaire couronné', ovas: 11, secret: true,
    desc: 'Remporter un titre avec trois clubs différents.',
    atteint: (c) => clubsTitres(c).size >= 3,
  },
  {
    id: 'fidele_trois_titres', emoji: '❤️', nom: 'Homme d’un seul club', ovas: 10,
  ...
  {
    id: 'gravir_pyramide', emoji: '🪜', nom: 'De la Fédérale à l’élite', ovas: 12, secret: true,
  ...
  {
    id: 'dix_titres', emoji: '🏛️', nom: 'Collectionneur', ovas: 12,
    desc: 'Remporter 10 titres en carrière.',
    atteint: (c) => c.joueur.titres.length >= 10,
  },
  {
    id: 'titre_a_letranger', emoji: '🌐', nom: 'Champion hors de France', ovas: 8,
    desc: 'Être champion d’un championnat étranger.',
    atteint: (c) => palmares(c).some((t) => TITRES_MONDE.has(t.trophee)),
  },
  {
    id: 'trois_saisons_de_suite', emoji: '🔁', nom: 'Trois de suite', ovas: 10, secret: true,
  ...
  {
    id: 'mille_points', emoji: '🎯', nom: 'Mille points', ovas: 6,
    desc: 'Inscrire 1 000 points en carrière.',
    atteint: (c) => (c.joueur.stats?.points ?? 0) >= 1000,
  },
  {
    id: 'buteur_precis', emoji: '🥅', nom: 'Le pied sûr', ovas: 6,
  ...
  {
    id: 'carton_rouge', emoji: '🟥', nom: 'Vingt minutes de folie', ovas: 2, secret: true,
    desc: 'Prendre un carton rouge.',
    atteint: (c) => (c.joueur.stats?.cartonsRouges ?? 0) >= 1,
  },
  {
    id: 'cent_essais', emoji: '⚡', nom: 'Machine à essais', ovas: 7,
  ...
  // ═══ LE PALMARÈS, SUITE ════════════════════════════════════════════════════
  {
    id: 'titre_saison_un', emoji: '🚀', nom: 'Tout de suite', ovas: 5, secret: true,
    desc: 'Remporter un titre dès ta première saison.',
    atteint: (c) => palmares(c).some((t) => t.saison === 1),
  },
  {
    id: 'trois_championnats', emoji: '🌐', nom: 'Champion sans frontières', ovas: 10, secret: true,
  ...
  {
    id: 'europe_complete', emoji: '⭐', nom: 'L’Europe au complet', ovas: 11, secret: true,
  ...
  // ═══ MÉTA ══════════════════════════════════════════════════════════════════
  {
    id: 'hall_trois', emoji: '🏛️', nom: 'Trois vies', ovas: 8,
    desc: 'Faire entrer trois carrières au Hall des Légendes.',
    atteint: (c) => c.pantheon.filter((l) => !l.fictif).length >= 3,
  },
  {
    id: 'presque_tout', emoji: '🧩', nom: 'Presque tout', ovas: 16, secret: true,
...
// ⚠️ ET SURTOUT : LEUR TOTAL EST PLAFONNÉ PAR SAISON
// (`PLAFOND_OVAS_DEFIS_PAR_SAISON`, store). C'était LA fuite de l'économie :
// 3 défis par semaine × 43 semaines × 2 Ovas = ~250 Ovas la saison, soit
// 3 000 sur une carrière — plus que tout le reste du jeu réuni. Les défis
// restent un moteur d'habitude (jouer sa semaine plutôt que sauter la saison) ;
// ils ne sont plus une machine à Ovas.
export const DEFIS: Defi[] = [
  { id: 'match', emoji: '👟', texte: 'Disputer un match cette semaine', ovas: 1 },
  { id: 'essai', emoji: '🏉', texte: 'Marquer un essai', ovas: 2 },
  { id: 'note7', emoji: '⭐', texte: 'Obtenir une note de 7 ou plus', ovas: 1 },
  { id: 'note8', emoji: '🌟', texte: 'Obtenir une note de 8 ou plus', ovas: 2 },
  { id: 'victoire', emoji: '✅', texte: 'Gagner avec ton club', ovas: 1 },
  { id: 'entrainement', emoji: '💪', texte: 'Faire ta séance d’entraînement', ovas: 1 },
  { id: 'post', emoji: '📱', texte: 'Publier sur L’Ovale', ovas: 1 },
  { id: 'plaquages', emoji: '🛡️', texte: 'Réussir 8 plaquages dans un match', ovas: 1 },
  { id: 'transformation', emoji: '🎯', texte: 'Réussir un tir au but', ovas: 1 },
