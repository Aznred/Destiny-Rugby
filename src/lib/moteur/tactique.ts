...
import {
  LARGEUR, borner, ligneDefendue, sens, distance, type Cote, type Vec,
} from './terrain';
...
export function placer(pions: Pion[], ctx: Contexte): void {
  ...
  for (const cote of ['A', 'B'] as Cote[]) {
    ...
    if (!attaque && ctx.porteur) {
      ...
      // 🛠️ CORRECTION DÉFENSE INTELLIGENTE
      [...liste]
          .filter((p) => p.recuperation <= 0) // N'envoie JAMAIS un joueur à terre
          .filter((p) => p.numero !== 15 || ctx.perceeEnCours) // Le 15 monte si ça perce
          .sort((a, b) => {
            // Le défenseur est-il battu ? (Dans le dos de l'attaquant)
            ...
            // Priorité absolue aux joueurs qui sont encore DEVANT le porteur
            if (aBattu !== bBattu) return aBattu ? 1 : -1;

            // Ensuite, c'est le plus proche qui y va
            return distance(a.pos, cible) - distance(b.pos, cible);
          })
          .slice(0, 2) // 2 joueurs montent pour sécuriser
