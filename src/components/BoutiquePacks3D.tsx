...
function Couronne({packs,mouvement,achat,surToucher,onSelection,gratuit}: {packs:PackCarriere[];mouvement:MutableRefObject<Mouvement>;achat:string|null;surToucher:(slot:number)=>void;onSelection:(n:number)=>void;gratuit:boolean}) {
  ...
  return <>{slots.map(slot=><Suspense key={slot} fallback={null}><Pochette pack={packs[modulo(slot,packs.length)]} slot={slot} mouvement={mouvement} achat={achat} surToucher={surToucher} gratuit={gratuit}/></Suspense>)}<mesh rotation={[-Math.PI/2,0,0]} position={[0,-1.82,-1.1]} scale={[1,.48,1]}><ringGeometry args={[4.9,4.93,96]}/><meshBasicMaterial color="#bdab68" transparent opacity={.25}/></mesh></>;
}
export default function BoutiquePacks3D({packs,solde,occupe,onOuvrir,gratuit=false}: {packs:PackCarriere[];solde:number;occupe:boolean;onOuvrir:(id:string,nom:string)=>Promise<void>;gratuit?:boolean}) {
   ...
   */
  async function acheter(slot:number){const p=packs[modulo(slot,packs.length)];if(!p||occupe||verrou.current)return;if(!gratuit&&solde<p.prix){setMessage(t('online.shop.missing',{n:nombre(p.prix-solde)}));return;}verrou.current=true;setAchat(p.id);setMessage(t('online.shop.opening'));mouvement.current.cible=slot;try{await onOuvrir(p.id,nomPackCarriere(p));}finally{verrou.current=false;if(vivant.current){setAchat(null);setMessage('');}}}
