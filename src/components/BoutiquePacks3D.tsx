...
import { modelePack, NOMS_PACK } from '../lib/presentationPacks';
...
class Protection extends Component<{children:ReactNode},{erreur:boolean}> {
  state={erreur:false}; static getDerivedStateFromError(){return {erreur:true};}
  render(){return this.state.erreur ? <p className="bp3-erreur">La scène 3D est indisponible. Les flèches du clavier permettent de choisir un pack, puis deux pressions sur Entrée de l’acheter.</p> : this.props.children;}
}
function Pochette({pack,slot,mouvement,achat,surToucher}: {pack:PackCarriere;slot:number;mouvement:MutableRefObject<Mouvement>;achat:string|null;surToucher:(slot:number)=>void}) {
  const {scene}=useGLTF(modelePack(pack.garantie ?? 'bronze'));
...
export default function BoutiquePacks3D({packs,solde,occupe,onOuvrir}: {packs:PackCarriere[];solde:number;occupe:boolean;onOuvrir:(id:string,nom:string)=>Promise<void>}) {
  ...
  function toucher(slot:number){if(performance.now()<ignorer.current||mouvement.current.glisse)return;const now=performance.now();if(dernierTap.current.slot===slot&&now-dernierTap.current.temps<430){dernierTap.current={slot:Infinity,temps:0};void acheter(slot);}else{dernierTap.current={slot,temps:now};setSelection(modulo(slot,packs.length));setMessage('');}}
  if(!pack)return <p>Aucun pack disponible.</p>;
  return <section className="bp3" aria-label="Boutique des packs 3D" style={{'--bp3-color':couleurs[pack.garantie??'bronze']} as CSSProperties}>
    <div className="bp3-scene" tabIndex={0} role="group" aria-label="Présentoir circulaire. Glisser pour tourner, double-cliquer sur un pack pour acheter. Au clavier : flèches, puis deux pressions sur Entrée." onPointerEnter={()=>{mouvement.current.pause=true;}} onPointerLeave={()=>{mouvement.current.pause=false;}} onFocus={()=>{mouvement.current.pause=true;}} onBlur={()=>{mouvement.current.pause=false;}} onPointerDown={e=>{if(e.button!==0)return;drag.current={x:e.clientX,depart:mouvement.current.cible,distance:0};mouvement.current.glisse=true;}} onPointerMove={e=>{if(!mouvement.current.glisse||mouvement.current.bloque)return;const dx=e.clientX-drag.current.x;drag.current.distance=Math.max(drag.current.distance,Math.abs(dx));if(Math.abs(dx)>6){e.currentTarget.setPointerCapture(e.pointerId);mouvement.current.cible=drag.current.depart-dx/150;}}} onPointerUp={e=>{mouvement.current.glisse=false;if(drag.current.distance>6){ignorer.current=performance.now()+300;dernierTap.current={slot:Infinity,temps:0};}if(e.currentTarget.hasPointerCapture(e.pointerId))e.currentTarget.releasePointerCapture(e.pointerId);}} onPointerCancel={()=>{mouvement.current.glisse=false;ignorer.current=performance.now()+300;}} onKeyDown={e=>{if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();mouvement.current.cible=Math.round(mouvement.current.cible)+(e.key==='ArrowRight'?1:-1);setSelection(modulo(mouvement.current.cible,packs.length));dernierTap.current={slot:Infinity,temps:0};}if(e.key==='Enter'&&!e.repeat){e.preventDefault();toucher(Math.round(mouvement.current.cible));}}}>
      <Protection><Canvas camera={{position:[0,1,8.8],fov:45}} dpr={[1,1.5]} gl={{alpha:true,antialias:true}}><ambientLight intensity={1.8}/><directionalLight position={[3,4,5]} intensity={3.5}/><directionalLight position={[-4,2,3]} intensity={2} color="#c7eaff"/><Couronne packs={packs} mouvement={mouvement} achat={achat} surToucher={toucher} onSelection={setSelection}/></Canvas></Protection>
      <span className="bp3-geste">Glisse pour faire tourner · Double-clique sur le pack pour acheter</span>
    </div>
    <div className="bp3-details"><div className="eyebrow">{pack.cartes} cartes · {pack.garantie?`${NOMS_PACK[pack.garantie]} garanti`:'Départ Bronze'}</div><h3>Pack {pack.nom}</h3><p>{pack.promesse??'De nouvelles recrues pour ton club.'}</p><strong className="bp3-prix">{pack.prix.toLocaleString('fr-FR')} <small>Ovas</small></strong><p className="bp3-message" role="status">{message||(solde<pack.prix?`Il te manque ${(pack.prix-solde).toLocaleString('fr-FR')} Ovas.`:'Double-clic pour acheter et ouvrir')}</p><details><summary>Chances de tirage</summary><ul>{Object.entries(pack.probabilites).map(([r,p])=><li key={r}>{NOMS_PACK[r as keyof typeof NOMS_PACK]} <b>{p.toLocaleString('fr-FR')} %</b></li>)}</ul></details></div>
  </section>;
}
