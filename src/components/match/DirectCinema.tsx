...
const libelleMoment=(type:string,texte:string)=>type==='essai'?'ESSAI':type==='carton'?(/rouge/i.test(texte)?'CARTON ROUGE':'CARTON JAUNE'):type==='penalite'||type==='faute'?'PÉNALITÉ':type==='but'?'TIR RÉUSSI':type==='butRate'?'TIR MANQUÉ':type==='blessure'?'BLESSURE':type==='remplacement'?'REMPLACEMENT':type==='franchissement'?'FRANCHISSEMENT':'ACTION IMPORTANTE';
export function DirectCinema({match:m,domicile,exterieur,couleurs,emblemes}:{match:VueMatchEnLigne;domicile:string;exterieur:string;couleurs:CouleursDirect;emblemes?:{domicile?:string;exterieur?:string}}) {
 ...
 const carton=momentVif?.type==='carton'?(/rouge/i.test(momentVif.texte)?'rouge':'jaune'):undefined;
 ...
 return <section className="dc" aria-label="Direct du match animé">
 <header className="dc-entete"><span><i/>{m.termine?'TERMINÉ':'EN DIRECT'} <b>MATCH ANIMÉ · 30 JOUEURS + ARBITRE</b></span><span>Terrain complet · animations rugby</span></header>
 ...
 <div className="dc-ecran">{m.terrain?<TerrainEnDirect key={m.id} terrain={m.terrain} nomDomicile={domicile} nomExterieur={exterieur} couleurs={couleurs} emblemes={emblemes} monCote={m.monCote} carton={carton}/>:<p className="dc-attente">{m.termine?'Match terminé. Retrouve toutes les actions ci-dessous.':'Les équipes prennent place…'}</p>}{(m.decision||momentVif)&&<div className={`dc-alerte-terrain dc-alerte-${m.decision?'penalite':momentVif?.type}`} role="status"><b>{m.decision?'PÉNALITÉ · DÉCISION':libelleMoment(momentVif!.type,momentVif!.texte)}</b><span>{m.decision?'Choisis entre les poteaux, la touche ou le jeu à la main.':momentVif!.texte}</span></div>}</div>
