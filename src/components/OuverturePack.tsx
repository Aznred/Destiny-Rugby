...
export default function OuverturePack({ cartes, pack, garantie, onFermer, rendreCarte }: {
  cartes: CarteCarriere[]; pack: string; garantie?: RareteCarriere; onFermer: () => void; rendreCarte: (carte: CarteCarriere) => ReactNode;
}) {
  const ordre = useMemo(() => [...cartes].sort((a,b) => rangPack(a)-rangPack(b) || a.note-b.note), [cartes]);
  const maximum = Math.max(0, ...cartes.map(rangPack));
  const [rang, setRang] = useState(Math.min(maximum, PALIERS_PACK.indexOf(garantie ?? 'bronze')));
  ...
  function passer() { setInstant(true); sons.arreter(); setRang(maximum); setPhase('cartes'); setRevelees(ordre.length); }
  useEffect(() => {
    const clavier = (e: KeyboardEvent) => {
      if (e.key.toLowerCase() === 'm') { sons.couper(!muet); setMuet(!muet); }
      if (e.key === 'Escape') { e.preventDefault(); if (phase === 'cartes' && toutes) onFermer(); else { setInstant(true); sons.arreter(); setRang(maximum); setPhase('cartes'); setRevelees(ordre.length); } }
    };
    window.addEventListener('keydown', clavier);
    if (!dialogue.current?.contains(document.activeElement)) principale.current?.focus();
    return () => window.removeEventListener('keydown', clavier);
  }, [phase, toutes, maximum, ordre.length, onFermer, muet, sons]);
  function action() {
    if (verrou.current || phase !== 'attente') return;
    sons.activer(); verrou.current = true;
  ...
  return createPortal(<div ref={dialogue} className={`pack-show phase-${phase} palier-${rarete}${calme ? ' calme' : ''}${instant ? ' instant' : ''}`} style={{ '--pack-color': COULEURS[rang] } as CSSProperties} role="dialog" aria-modal="true" aria-labelledby="pack-show-title" onKeyDown={e => {
    ...
    <div className="pack-show-heading"><p className="eyebrow">Pack {pack} · {cartes.length} cartes</p><h2 id="pack-show-title" key={`${phase}-${rang}`} aria-live="polite">{phase === 'cartes' ? 'Tes nouvelles recrues' : NOMS_PACK[rarete]}</h2></div>
    ...
    </div><p className="pack-show-hint" aria-live="polite">{phase === 'ouverture' ? 'Ouverture…' : phase === 'charge' ? 'Ça monte…' : phase === 'evolution' ? NOMS_PACK[rarete]+' !' : 'Touche le pack'}</p></> : <div className="pack-show-results">{ordre.map((carte,i) => <div key={carte.id} className={`pack-show-card ${i<revelees?'visible':''} ${i===ordre.length-1?'meilleure':''}`}>
